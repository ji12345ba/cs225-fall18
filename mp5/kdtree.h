...
class KDTree
{
  ...
  private:
    ...
    KDTreeNode *root;
    size_t size;
    //vector to store the input list of points
    ...
    void _build_tree(int start, int end, int dimension, KDTreeNode *& subRoot);

    //Quick select to find medium
    void _quick_select(int start, int end, int k, int dimension);

    //helper to partition the vector
    int _partition(int start, int end, int dimension);

    KDTreeNode * _copy(const KDTreeNode *& node);

    void _clear(KDTreeNode *& root);

    void _findNearestNeighbor_helper(const Point<Dim> & query, Point<Dim> & curr_best,
      double & best_dist, KDTreeNode * subRoot, int curr_dim) const;

    // void _findNearestNeighbor_helper(const Point<Dim> & query, Point<Dim> & currBest,
      //     int start, int end, int dimension, double & distance, bool & first) const;

     double _getDistance(const Point<Dim>& p1, const Point<Dim>& p2) const;

     bool _check_other_side(const Point<Dim>& query,
